...
void heapWalk( char * caption)
{
	...
	do
	{
		...
		int blockheader;
		memcpy(&blockheader, blockstart, sizeof(blockheader));
		...
		int blockoffset = (((int)blockstart - (int)base)/8); 
		...
		if (STATUS == 0) {
			printf("    %d  FREE   %d\t%d\t  %d bytes   %d bytes\t   %d\n", blockNum, (int)blockstart, blockoffset, payload, 2*sizeof(blockheader), blocksize);
		} else {
			printf("    %d  USED   %d\t%d\t  %d bytes   %d bytes\t   %d\n", blockNum, (int)blockstart, blockoffset, payload, 2*sizeof(blockheader), blocksize);
		}
